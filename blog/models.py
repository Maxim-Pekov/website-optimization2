...
class PostQuerySet(models.QuerySet):
    ...
    def fetch_with_comments_count(self):
        """Функция создает псевдо поле comments_count, которое нет в БД. Но к этому полю можно обращаться на уровне
        питона. Этот спопсоб лучше чем использования двух annotate, т.к. они перегрудают страницу и загрузка может
        затянуться на несколько секунд"""
    ...
    def prefetch_posts_with_comments_count_tags_author(self):
        return self.prefetch_related('author').prefetch_with_tags().fetch_with_comments_count()
...
class Post(models.Model):
    ...
    def image_tag(self):
        if self.image:
            return mark_safe('<img src="%s" style="width: 120px;" />' % self.image.url)
        else:
